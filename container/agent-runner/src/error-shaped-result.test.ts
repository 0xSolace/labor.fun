...
describe("isErrorShapedResult", () => {
  ...
  it("classifies every rate-limit shape the bundled CLI builds", () => {
    ...
    for (const kind of [
      ...
      expect(isErrorShapedResult(`You've hit your ${kind}${resets}`)).toBe(true);
      expect(isErrorShapedResult(`You've hit your ${kind}`)).toBe(true);
    }
    expect(isErrorShapedResult(`You're out of extra usage${resets}`)).toBe(true);
  ...
  it("does NOT flag a reply that only starts like a limit notice", () => {
    ...
    expect(isErrorShapedResult("you've hit your limit for hearts this month")).toBe(
      false,
    );
  });
