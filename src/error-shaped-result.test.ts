...
describe('error-shaped-result duplication', () => {
  // container/agent-runner is a separate TS build (its own rootDir) and can't
  // import from src/, so this module exists twice: the container classifies
  // each result and the host re-checks before posting. A pattern added to one
  // copy but not the other silently weakens that side, so keep them identical.
  it('is byte-identical to the container/agent-runner copy', () => {
    const read = (rel: string) =>
      readFileSync(new URL(rel, import.meta.url), 'utf8');
    expect(read('../container/agent-runner/src/error-shaped-result.ts')).toBe(
      read('./error-shaped-result.ts'),
    );
  });
});
